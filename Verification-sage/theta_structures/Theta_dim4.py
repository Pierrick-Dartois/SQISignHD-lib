...
class ThetaStructureDim4:
	...
	def _arithmetic_base_change(self,max_iter=50):
		O=self.null_point()
		HO=hadamard(O)
		HSHO=hadamard(squared(HO))
		if all([HSHO[k]!=0 for k in range(16)]) and all([HO[k]!=0 for k in range(16)]):
			self._arith_base_change_matrix=None
			self._arith_base_change_matrix_inv=None
			self._inv_null_point=batch_inversion(HO)
			self._inv_null_point_dual_sq=batch_inversion(HSHO)
		else:
			F=self._base_ring
			if F.degree() == 2:
	...
	def apply_arith_base_change(self,P):
		if self._arith_base_change:
			if self._arith_base_change_matrix is None:
				return hadamard(P)
			else:
				return apply_base_change_theta_dim4(self._arith_base_change_matrix,P)
		else:
			return P

	def apply_arith_base_change_inverse(self,P):
		if self._arith_base_change:
			if self._arith_base_change_matrix_inv is None:
				return hadamard(P)
			else:
				return apply_base_change_theta_dim4(self._arith_base_change_matrix_inv,P)
		else:
			return P
...
class ThetaPointDim4:
    ...
    def double(self,apply_arith_base_change=True):
    	...
    	if apply_arith_base_change and self.parent()._arith_base_change:
    		P=self.parent().apply_arith_base_change(P)#apply_base_change_theta_dim4(self.parent()._arith_base_change_matrix,P)
    	...
    	if apply_arith_base_change and self.parent()._arith_base_change:
    		theta_2P=self.parent().apply_arith_base_change_inverse(theta_2P)#apply_base_change_theta_dim4(self.parent()._arith_base_change_matrix_inv,theta_2P)
    ...
    def double_iter(self,n):
    	...
    	if self.parent()._arith_base_change:
    		Q=ThetaPointDim4(self.parent(),self.parent().apply_arith_base_change(self))
    		for i in range(n):
    			Q=Q.double(apply_arith_base_change=False)
    		Q=ThetaPointDim4(self.parent(),self.parent().apply_arith_base_change_inverse(Q))
    	else:
    		Q=self
    		for i in range(n):
    			Q=Q.double()
