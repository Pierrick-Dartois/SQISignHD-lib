...
- `SIDH_attack.py` to test SIDH attacks as described in [5].

To use the CLI, type:

`sage <Tests.py/Verify_SQISignHD.py/SIDH_attack.py> <arguments>`
...
The main test files `Tests.py`, `Verify_SQIsignHD.py` and `SIDH_attack.py` are located in the main directory `Verification-sage` of this library.
...
- `SIDH` contains data to run the SIDH attack.
...
[5] D. Robert. Breaking SIDH in polynomial time. In Advances in Cryptology - EUROCRYPT 2023. https://eprint.iacr.org/2022/1038.
