...
xisog_3(ec_kps_t *kps, ec_point_t *B24, const ec_point_t P)
{
    ...
    fp2_sub(&B24->z,&r5,&B24->x); // C24=r5-A24=-16xz^3. OK
    */

    fp2_add(&r1,&P.x,&P.x); // 2x
    fp2_add(&r1,&r1,&P.x); // 3x
    fp2_sub(&r1,&P.z,&r1); // z-3x
    fp2_sqr(&r2,&r1); // (z-3x)^2
    fp2_mul(&r1,&r2,&r1); // (z-3x)^3
    fp2_mul(&B24->x,&r1,&K.z); // (x+z)(z-3x)^3
    fp2_sqr(&r1,&P.z); // z^2
    fp2_mul(&r1,&r1,&P.z); // z^3
    fp2_mul(&r1,&r1,&P.x); // x*z^3
    fp2_add(&r1,&r1,&r1); // 2xz^3
    fp2_add(&r1,&r1,&r1); // 4xz^3
    fp2_add(&r1,&r1,&r1); // 8xz^3
    fp2_add(&r1,&r1,&r1); // 16xz^3
    fp2_neg(&B24->z,&r1); // -16xz^3
}
