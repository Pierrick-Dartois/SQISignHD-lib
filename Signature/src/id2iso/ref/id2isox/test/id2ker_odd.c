...
static void
random_pair_of_equivalent_ideals(quat_left_ideal_t *I, quat_left_ideal_t *J)
{
    ibz_t normIJ;
    ibz_init(&normIJ);
...
_id2iso_test_id2kerdlogs_odd()
{
    ...
    void id2iso_ideal_to_kernel_dlogs_odd(
        ibz_vec_2_t * vec, ec_degree_odd_t * deg, const quat_left_ideal_t *lideal);
    ...
    //    printf("deg1 = "); for (size_t i = 0; i < sizeof(deg1)/sizeof(*deg1); ++i)
    //    printf(",%d"+(!i), (int) deg1[i]); printf("\n"); printf("deg2 = "); for (size_t i = 0; i <
    //    sizeof(deg2)/sizeof(*deg2); ++i) printf(",%d"+(!i), (int) deg2[i]); printf("\n");
...
_id2iso_test_id2ker_odd()
{
    ...
    ec_isog_odd_t isog1, isog2;
    {
        ...
        // check orders
        {
            if (!ibz_is_one(&ideal1.norm) && fp2_is_zero(&isog1.ker_plus.z) &&
                fp2_is_zero(&isog1.ker_minus.z)) {
